...
/**
 * icon — 32×32 PNG favicon generated at the edge, picked up
 * automatically by Next App Router and emitted as
 * `<link rel="icon" />` in the head.
 *
 * Same white frangipani mark on brand-ink background as the apple
 * touch icon, so the browser-tab favicon matches the rest of the
 * visual system. Replaces the default create-next-app favicon.ico.
 */
...
export default async function Icon() {
  return new ImageResponse(
    (
      ...
      >
        <svg width="22" height="21" viewBox="0 0 232.34 220.98">
          <g fill="#FFFFFF">
            {PETAL_PATHS.map((d, i) => (
              <path key={i} d={d} />
            ))}
